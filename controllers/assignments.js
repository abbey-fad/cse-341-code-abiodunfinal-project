...
const getSingle = async (req, res) => {
	...
	if (!ObjectId.isValid(req.params.id)) {
		return res.status(400).json('Must use a valid assignment id.');
	}
...
const createAssignment = async (req, res) => {
	...
	try {
		...
		if (response.acknowledged) {
			res.status(201).json({ _id: response.insertedId, ...assignment });
		} else {
			res.status(500).json(response.error || 'Error creating assignment.');
		}
	} catch (err) {
		res.status(500).json({ message: err.message });
	}
